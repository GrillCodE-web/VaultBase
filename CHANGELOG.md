...
> **Важно: эту версию тоже нужно поставить вручную один раз** — в 2.12.2 (и всех релизах раньше) скачивание обновления блокировалось ACL: в capabilities не было разрешения `updater:allow-download-and-install`, поэтому `downloadAndInstall()` из UI отбивался ошибкой «not allowed by ACL». Скачать: панель релизов / GitHub Releases. Начиная с 2.12.3 обновления снова ставятся из приложения.

### Fixed

- **Критично: авто-обновление не могло скачать пакет**. В `src-tauri/capabilities/default.json` добавлено `updater:allow-download-and-install` — отдельное разрешение на атомарную команду `plugin:updater|download_and_install` (tauri-plugin-updater 2.10.1 держит её отдельно от allow-download/allow-install). Баг существовал со дня появления авто-апдейтера; capabilities запечены в бинарь, поэтому старые установки самообновиться не могут — нужна одна ручная переустановка.

## [2.12.2] — 2026-09-05

> **Важно: эту версию нужно поставить вручную один раз** — в 2.12.1 сломан сам авто-апдейтер (см. ниже), он не увидит это обновление. Скачать: панель релизов / GitHub Releases. Дальше обновления снова пойдут автоматически.

### Added

- **E2E-чат работник ↔ менеджер** (MGR-016 frontend): страница «Чат» в обоих приложениях — список комнат слева, лента сообщений справа, бейдж «E2E X25519+AES-GCM» в шапке, pull-to-refresh, автопрочтение, живые события без перезагрузки страницы. Сервер — opaque-relay: в сети только X25519-sealed конверты, расшифровка исключительно на устройствах. Хоткей переключения: `G T`.
- **Аудит инфраструктуры одним скриптом**: `scripts/audit-infrastructure.py` — TLS/версия/апдейтер (с проверкой файлов артефактов), 20 API-роутов, WS-рукопожатие + nonce (MGR-008), deep-режим с токеном: auth→auth_ok и sync-роуты.

### Fixed

- **Критично: авто-апдейтер молчал навсегда**. Tauri v2 `check()` возвращает объект Update или `null`, поля `.available` у него нет — а три места (автопроверка каждые 6 ч, баннер при открытии страницы обновлений, ручная кнопка «Проверить сейчас») проверяли именно его. Любое доступное обновление отбрасывалось; ручная проверка всегда отвечала «вы на последней версии».
- **Синхронизация: убран «групповой хвост»**. После выпила групп (MGR-018) в воркере остался фоновый cron и модуль, каждые 5 минут словшие `POST /sync/group/stats` на сервер и получавшие `409 no_sync_group` — спам в логах и ложное ощущение «не синхронизируется». Мёртвый код удалён целиком.

### Notes

- Одновременно выпущен **VaultBase Manager 0.2.2** — то же (чат + команды) в втором приложении.
...
- **MGR-011 — персональная стата работника и умная лента**: новая команда `get_worker_stats` агрегирует расшифрованные daily_stats по конкретному работнику (заказы по статусам, карты, дохлость, дропы, health ок/фейл) и строит ленту событий по правилам: спайк деклайнов, мёртвые карты, падение IMAP/SMTP/прокси, чистые дни и их серии, высокий объём, тихие дни. В UI: сортировка списка работников (активность/имя/статус) и блок статистики с лентой в карточке работника; 5 новых Rust-тестов.
- **MGR-006 — новости менеджера и приоритеты шопов в воркере**: миграция v17 (таблицы `manager_news` и `shop_priorities`); heartbeat best-effort подтягивает ленты `/api/telemetry/news` и `/api/telemetry/priorities`; новые команды `manager_refresh_feeds`, `get_manager_news`, `mark_manager_news_read`, `get_shop_priorities`. В UI: компонент NewsAlert — баннеры над контентом по severity (info/warning/error) с закрытием = отметкой о прочтении; вкладка Shops каталога получает колонку «Приоритет» (★ вес) и сортировку по весу из менеджерских приоритетов; 4 новых Rust-теста.
- **MGR-007 — движок числовых алертов в менеджер-аппе**: новый модуль `alerts.rs` считает правила на расшифрованных daily_stats локально (спайк деклайна с порогом и эскалацией в critical ≥70%, доля мёртвых карт, достижение дневных квот карт/заказов из политики) и пишет в локальную таблицу `local_alerts` с дедупликацией `{iid}:{категория}:{дата}` — на сервер ничего не уходит, отчёты по-прежнему E2E. Квоты синкатся из `/manager/api/workers` в `worker_snapshots`. Страница Alerts мержит серверные и локальные алерты (тег «локал»); в Settings — секция порогов, опциональный generic webhook (HTTPS POST пачкой новых алертов, без Telegram) и автоблок по idle-таймеру; OS-уведомления через tauri-plugin-notification — только в активной сессии. Новые команды: `evaluate_alerts`, `get_local_alerts`, `local_alert_action`, `get_config_values`, `set_config_value` (whitelist). 6 новых Rust-тестов (24/24 green), eslint 0, build ok.
- **MGR-008 — сервер-харденинг**: лицензионные токены и `user_token` в footprints хранятся только как SHA-256 хеши (миграция v13 с бэкфиллом; открытый токен выдаётся один раз при активации/ротации и в БД не попадает, в админке — маскированный префикс хеша). Kill-switch деплоя (`server_config.kill_switch`): воркерские каналы (sync/footprint/WS/heartbeat) и раздача обновлений отвечают 503 `service_halted`, менеджерские каналы и админка продолжают работать. Анти-replay WS: сервер первым фреймом шлёт `auth_challenge` с одноразовым nonce, при включённом `ws_require_nonce` auth обязан эхом вернуть nonce (по умолчанию выкл — legacy-клиенты работают; воркер ждёт challenge с таймаутом и откатывается в legacy-режим на старых серверах). Управление флагами: `<ADMIN_PATH>/api/server-config` + секция «Аварийные выключатели» в админ-панели, изменения аудируются. Серверные тесты: 79/79 green (новые: nonce anti-replay, kill-switch на WS); Rust: 152/152 green.
...
- **PBKDF2 итерации подняты до 1 000 000** (было 600 000). Соответствует OWASP рекомендации «высокая безопасность». `encryption.rs` теперь использует константу `PBKDF2_ITERATIONS` вместо хардкода.

### Улучшения

- **Пресеты фильтров карт** — сохранение именованных комбинаций фильтров (статус + страна + банк + ...) через кнопку ⚡ в панели фильтров. Хранятся в localStorage между сессиями.
- **Предупреждение об истечении сессии** — баннер с обратным отсчётом появляется за 2 минуты до автоблокировки. Сбрасывается при любой активности пользователя.
- **Сообщения об ошибках на русском с подсказками** — каждый тип ошибки (сеть, БД, шифрование, валидация, Stuffer, права) теперь содержит поле `suggestion` с рекомендацией действий. Новые хелперы `getErrorTitle()` / `getErrorSuggestion()`.
- **Персистентное состояние UI** — новый хук `usePersistedState` для автосохранения UI-настроек в localStorage с версионированием схемы. Применён к видимости и порядку колонок в Cards.
- **Auto-retry с exponential backoff для Stuffer API** — `get()`/`post_json()` делают до 3 попыток (500мс → 1с → 2с) при сетевых ошибках и HTTP 5xx.
- **Валидация Stuffer UI** — поле `shop` обязательно, `pay_option` заменён на `<select>` с фиксированными значениями.

### Исправления

- **Мёртвые секции конфигурации удалены** — `[logging]`, `[api]`, `[database]`, `[email]` убраны из TOML-профилей и `config.rs`. Код никогда их не читал, а пользователей они вводили в заблуждение. `endpoints.rs` переведён с `config.api.sync_server` на константу.
- **Неиспользуемый импорт `safeGetJSON`** удалён из `App.jsx`.
- **eslint `no-console`** отключён для `logger.js` — убраны 5 ложных предупреждений.
- **ship.py** — защита от нераскрытых шаблонов усилена: теперь отлавливает любой `%VAR%`, а не только `%GIT_MSG%`.

### Тесты

- **12 unit-тестов Stuffer API** — покрытие `build_url`, парсинг `CourierFull`/`Package`/`LabelFile`, сериализация `PackageInput`, дефолты, ошибки отсутствующих полей.
- Тест PBKDF2 обновлён под 1M итераций.
- Тест `getErrorMessage` обновлён под русские сообщения.

### Гигиена репозитория

- 28 отчётных файлов (`SPRINT3_*.md`, `*_REPORT.txt` и т.п.) перемещены в `archive/`.
- 4 ad-hoc скрипта анализа удалены.
- Удаления `README.md`, `AGENTS.md`, `CHECKLIST.md`, `PROJECT_STATUS.md` и неиспользуемых модулей закоммичены.
- `.github/workflows/test.yml` добавлен в git.
...
- Исправленные мелкие баги / Переписан UI под одну стилизацию
...
- Обновление в один клик + уведомления о новых версиях
