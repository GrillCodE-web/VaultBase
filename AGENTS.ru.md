...
раздел «Оборванная сессия» в PARALLEL_WORK.md. Непрерывность сессий: в начале
сессии читай хвост SESSION_LOG.md, после каждого закрытого пункта чеклиста
добавляй запись — чаты обрываются, журнал остаётся.
