...
import {
  ...
  RefreshCw, X, Upload, Eye, EyeOff, Store, CheckCircle2, Wifi, WifiOff, Loader2,
...
function TypeBadge({ type }) {
  const colorMap = {
    http:   { color: STATUS_COLORS.info, bg: "rgba(96,165,250,0.12)" },
...
function UsageStatsModal({ onClose }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-md)" }} role="dialog" aria-modal="true" aria-labelledby="proxy-stats-title">
        <div className="flex items-center justify-between mb-[18px]">
          <div id="proxy-stats-title" className="modal-title" style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: 32, color: "var(--muted)", fontSize: 13 }}>
            <Loader2 size={18} className="animate-spin" style={{ display: "inline-block", marginBottom: 8 }} />
        ...
        {!loading && stats && stats.length > 0 && (
          <table className="tbl">
            ...
            <tbody>
              {stats.map((s) => {
                const rate = s.total_orders > 0 ? Math.round((s.success_count / s.total_orders) * 100) : 0;
                ...
                return (
                  <tr key={s.proxy_id}>
                    ...
                    <td>
                      ...
                      {rate < 40 && (
                        <span style={{ marginLeft: 8, fontSize: 10, color: STATUS_COLORS.error, display: "inline-flex", alignItems: "center", gap: 3 }}>
...
function ProxyModal({ initial, onSave, onClose }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-sm)" }} role="dialog" aria-modal="true" aria-labelledby="proxy-modal-title">
        <div className="flex items-center justify-between mb-[18px]">
          <div id="proxy-modal-title" className="modal-title">{isEdit ? "Edit Proxy" : "Add Proxy"}</div>
          <button onClick={onClose} className="modal-close" aria-label="Close"><X size={16} /></button>
        </div>
        <div className="flex flex-col gap-3.5">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 10 }}>
            <div className="form-group">
              ...
              <input value={form.host} onChange={set("host")} placeholder="proxy.example.com" className="form-input mono" />
            </div>
            <div className="form-group">
              <label className="form-label">Port *</label>
              <input type="number" value={form.port} onChange={set("port")} placeholder="8080" className="form-input mono" />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div className="form-group">
              ...
              <input value={form.username} onChange={set("username")} placeholder="user" className="form-input mono" />
            </div>
            <div className="form-group">
          ...
          <div className="form-group">
            ...
            <input value={form.label} onChange={set("label")} placeholder="Residential US" className="form-input" />
          </div>
          <div className="form-group">
          ...
          <button
            ...
            className="btn btn-b"
            style={{ width: "100%", opacity: !valid || loading ? 0.4 : 1, cursor: !valid || loading ? "not-allowed" : "pointer" }}
          >
...
function ImportModal({ onDone, onClose }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-md)" }} role="dialog" aria-modal="true" aria-labelledby="import-proxies-title">
        <div className="flex items-center justify-between mb-[18px]">
          <div id="import-proxies-title" className="modal-title">Import Proxies</div>
          <button onClick={onClose} className="modal-close" aria-label="Close"><X size={16} /></button>
        </div>
        <div className="flex flex-col gap-3.5">
          {!result ? (
            <>
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, padding: "10px 14px", fontSize: 11, color: "var(--muted)", fontFamily: "JetBrains Mono, monospace" }}>
                <div style={{ fontWeight: 600, marginBottom: 6, color: "var(--text-secondary)" }}>Supported formats:</div>
            ...
            <div className="flex flex-col gap-3.5">
              <div className="grid grid-cols-2 gap-3">
                <div style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 10, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: STATUS_COLORS.success }}>{result.parsed}</div>
                  <div className="text-[11px] text-muted mt-1">{t("cc_import_done")}</div>
                </div>
                <div style={{ background: "rgba(234,179,8,0.08)", border: "1px solid rgba(234,179,8,0.2)", borderRadius: 10, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 28, fontWeight: 700, color: STATUS_COLORS.warning }}>{result.skipped}</div>
                  <div className="text-[11px] text-muted mt-1">{t("profiles_skipped")}</div>
                </div>
              </div>
              {result.errors?.length > 0 && (
                <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, padding: 10, maxHeight: 120, overflowY: "auto" }}>
                  {result.errors.map((e, i) => (
                    <div key={i} className="mono" style={{ fontSize: 11, color: STATUS_COLORS.error, padding: "2px 0" }}>{e}</div>
                  ))}
                </div>
              )}
              <button
    ...
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      ...
      {open && (
        <div style={{
          ...
          {currentBinding && (
            <button
              ...
              style={{
                ...
                fontSize: 11, color: STATUS_COLORS.error, background: "transparent",
          ...
          {shops.length === 0 && (
            <div style={{ padding: "8px 12px", fontSize: 11, color: "var(--muted)" }}>Loading shops…</div>
          )}
          {shops.map(s => (
            <button
              key={s.id}
...
export default function ProxyList() {
  ...
  const [testAllProgress, setTestAllProgress] = useState({ current: 0, total: 0, online: 0, failed: 0 });
  ...
  const load = useCallback(async (p = page, fb = filterBlocked, ft = filterType, fu = filterUsed) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      invoke("get_shops", { page: 1, perPage: 500, search: "" }).then(r => r.items || []).catch(() => []),
    ]).then(([bindings, shops]) => {
      const cache = {};
      shops.forEach(s => { cache[s.id] = s; });
      setShopCache(cache);
      const bindMap = {};
      bindings.forEach(b => {
  ...
  const handleDelete = async (proxy) => {
    const ok = await confirm(t("proxy_confirm_delete") + ` ${proxy.host}:${proxy.port}?`, { danger: true });
    if (!ok) return;
    try {
  ...
  const handleTestAll = async () => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        ...
        if (ok) { online++; setTestResults(prev => ({ ...prev, [p.id]: "online" })); }
        else    { failed++; setTestResults(prev => ({ ...prev, [p.id]: "failed" })); }
      } catch {
        failed++;
  ...
  const handleTestProxy = async (proxy) => {
    ...
    try {
      ...
      toast(ok ? `${proxy.host}:${proxy.port} — reachable` : `${proxy.host}:${proxy.port} — unreachable`, ok ? "success" : "error");
    } catch (e) {
  ...
  const handleCheckNow = async () => {
    ...
    try {
      ...
      toast(`Checked ${result.checked} proxies: ${result.online} online, ${result.offline} offline`, "success");
      load();
    } catch (e) {
      toast(String(e), "error");
    } finally {
  ...
  return (
    <div className="content">
      ...
      <div className="ph">
        <div>
          <div className="ph-title"><Globe size={14} /> {t("proxy_manager")}</div>
        ...
        <div className="ph-actions">
          ...
          >
            ...
            {testingAll ? `${t("btn_test")} ${testAllProgress.current}/${testAllProgress.total}…` : t("proxy_test_all")}
          </button>
          <button
            className="btn btn-ghost btn-sm"
          ...
          >
            <Wifi size={13} style={{ animation: checkingHealth ? "spin 1s linear infinite" : "none" }} />
      ...
      <div className="filters mb-3">
        ...
        {[["http", "HTTP"], ["socks5", "SOCKS5"], ["socks4", "SOCKS4"], ["pptp", "PPTP"]].map(([val, label]) => (
          <button
            key={val}
      ...
      <div className="panel p-0">
        ...
        <table className="tbl">
          ...
          <tbody>
            {proxies.length === 0 && loading && <SkeletonRows count={6} cols={7} />}
            {proxies.length === 0 && !loading && (
              <EmptyState
                ...
                action={<button className="btn btn-g btn-sm" onClick={() => setModal("add")}><Plus size={12} /> Add Proxy</button>}
              />
            )}
            {proxies.map((proxy) => {
              // Derive health status from testResults for the row
              ...
              // G2: current shop binding for this proxy
              ...
              return (
                <tr key={proxy.id} style={{ opacity: proxy.is_blocked ? 0.6 : 1 }}>
                  <td style={{ color: "var(--text-secondary)" }}>
                    ...
                    {boundShop && (
                      <span style={{
                        ...
                        background: "rgba(96,165,250,0.12)", color: STATUS_COLORS.info,
                        border: "1px solid rgba(96,165,250,0.25)", fontFamily: "JetBrains Mono, monospace",
                      }}>
                  ...
                  <td className="mono text-[11px] text-muted">
                    {proxy.host}<span style={{ color: "var(--border)" }}>:</span>{proxy.port}
                  </td>
                  <td><TypeBadge type={proxy.proxy_type} /></td>
                  <td><StatusBadge proxy={proxy} healthStatus={healthStatus} /></td>
                    ...
                    <div className="tbl-actions">
                      ...
                      >
                        {(testingId === proxy.id || testResults[proxy.id] === "testing")
                          ...
                          : (testResults[proxy.id] === true || testResults[proxy.id] === "online")
                            ? <Wifi size={12} style={{ color: STATUS_COLORS.success }} />
                            : (testResults[proxy.id] === false || testResults[proxy.id] === "failed")
                              ? <WifiOff size={12} className="text-red-t" />
                              : <Wifi size={12} />}
                      </button>
                      {/* G2: Bind to Shop button */}
                      <BindToShopDropdown
                        proxy={proxy}
                        currentBinding={boundShop}
                        onBound={(shop) => setProxyBindings(prev => ({ ...prev, [proxy.id]: shop }))}
                        onUnbound={() => setProxyBindings(prev => { const n = { ...prev }; delete n[proxy.id]; return n; })}
                      />
                      <button className="btn btn-ghost btn-sm" onClick={() => setModal(proxy)}>Edit</button>
                      <button className="btn btn-r btn-sm" onClick={() => handleDelete(proxy)}>{t("btn_delete")}</button>
                    </div>
                  </td>
                </tr>
              );
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex gap-1">
            {buildPageNumbers(page, totalPages).map((p, idx) => (
              p === '…' ? (
                <span key={`ellipsis-${idx}`} className="btn btn-sm btn-ghost" style={{ cursor: "default" }}>…</span>
              ) : (
                <button
                  key={p}
                  onClick={() => { setPage(p); load(p, filterBlocked, filterType, filterUsed); }}
      ...
      {modal === "stats" && (
        <UsageStatsModal onClose={() => setModal(null)} />
      )}
