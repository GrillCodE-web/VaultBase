...
import {
  ...
  Mail, CheckCircle, AlertCircle, Package, X, Send, ChevronDown,
  ChevronRight, Folder, FolderOpen, Settings, PenSquare,
  AlertOctagon, FileText, Database, CornerUpLeft, Archive, Search,
} from "lucide-react";
import { useLang } from "../hooks/useLang";
import { useToast } from "../hooks/useToast";
import { useConfirm } from "../hooks/useConfirm";
import { SkeletonRows } from "../components/SkeletonRow.jsx";
import { IMAP_PROVIDERS, SMTP_PROVIDERS, detectImapConfig, detectSmtpConfig } from "../constants/emailProviders.js";
...
function AccountModal({ account, onSave, onClose }) {
  ...
  const [smtpForm, setSmtpForm] = useState({ host: "", port: 587, use_tls: false, use_starttls: true });
  ...
  const handleLoginChange = (email) => {
    ...
    if (imapCfg && !isEdit) {
      setForm(p => ({ ...p, login: email, label: p.label || email.split("@")[0], host: imapCfg.host, port: imapCfg.port }));
    ...
    if (smtpCfg && !isEdit) {
      ...
      setSmtpForm({ host: smtpCfg.host, port: smtpCfg.port, use_tls: smtpCfg.use_tls, use_starttls: smtpCfg.use_starttls });
    } else {
  ...
  const handleTest = async () => {
    if (!isEdit) { setTestResult({ ok: false, msg: t("imap_test_save_first") }); return; }
  ...
  const handleSave = async () => {
    if (!form.label || !form.host || !form.login) { toastErr(t("imap_fields_required")); return; }
    setSaving(true);
    try {
      await onSave(form);
      ...
      if (!isEdit && setupSmtp && smtpDetected && form.password) {
        ...
        } catch (_) { /* SMTP save failed — non-critical */ }
      }
      onClose();
    }
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-md)" }} role="dialog" aria-modal="true" aria-labelledby="imap-account-title">
        <div className="flex items-center justify-between mb-[18px]">
          <div id="imap-account-title" className="modal-title">{account ? t("imap_edit_account") : t("imap_add_account")}</div>
        ...
        <div className="flex flex-col gap-3">
          <div className="form-group">
            ...
            <input type="text" value={form.login} onChange={(e) => handleLoginChange(e.target.value)}
              placeholder="user@yahoo.com" className="form-input" />
            {autoDetected && (
              <div style={{ marginTop: 4, fontSize: 11, color: "#4ade80" }}>
                ✓ {t("imap_auto_configured")}: {autoDetected}:{form.port} — {t("imap_use_app_password")}
              </div>
            )}
          </div>
          {[
            { label: t("col_label"), key: "label", type: "text", placeholder: "Yahoo work" },
            { label: t("col_host_port")?.split(":")[0] ?? "Host", key: "host", type: "text", placeholder: "imap.mail.yahoo.com" },
            { label: t("imap_port"), key: "port", type: "number", placeholder: "993" },
            { label: t("imap_app_password"), key: "password", type: "password", placeholder: account ? t("imap_leave_blank") : t("imap_app_password_hint") },
            { label: t("imap_poll_interval"), key: "poll_interval", type: "number", placeholder: "60" },
        ...
        {!isEdit && smtpDetected && (
          <div style={{ marginTop: 12, padding: "10px 12px", borderRadius: 8, background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.15)" }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
              <input type="checkbox" checked={setupSmtp} onChange={e => setSetupSmtp(e.target.checked)} />
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          ...
          <button onClick={onClose} className="btn btn-ghost btn-sm">{t("btn_cancel")}</button>
...
function SmtpModal({ onSave, onClose }) {
  ...
  const [form, setForm] = useState({ label: "", host: "", port: 587, login: "", password: "", use_tls: false, use_starttls: true });
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState(null);
  const [saving, setSaving] = useState(false);
  ...
  const handleTest = async () => {
    if (!form.host || !form.login || !form.password) { setTestResult({ ok: false, msg: t("imap_fields_required") }); return; }
    setTesting(true); setTestResult(null);
    try {
      // Test by saving temporarily — we'll skip and just report "save first"
      setTestResult({ ok: false, msg: "Save the config first, then use Test Connection" });
    } finally { setTesting(false); }
  };

  const handleSave = async () => {
    if (!form.label || !form.host || !form.login || !form.password) { toastErr(t("imap_fields_required")); return; }
    setSaving(true);
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-md)" }} role="dialog" aria-modal="true" aria-labelledby="smtp-modal-title">
        <div className="flex items-center justify-between mb-[18px]">
          <div id="smtp-modal-title" className="modal-title">{t("imap_add_smtp")}</div>
          <button onClick={onClose} className="modal-close" aria-label="Close"><X size={16} /></button>
        </div>
        <div className="flex flex-col gap-3">
          {[
...
function ComposeModal({ smtpConfigs, defaultTo, defaultSubject, defaultBody, onClose, onSent }) {
  ...
  const handleSend = async () => {
    if (!form.smtp_config_id) { toastErr(t("imap_select_smtp")); return; }
    if (!form.to || !form.subject) { toastErr(t("imap_to_subject_required")); return; }
    setSending(true);
    try {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-lg)" }} role="dialog" aria-modal="true" aria-labelledby="compose-modal-title">
        <div className="flex items-center justify-between mb-[18px]">
          <div id="compose-modal-title" className="modal-title"><PenSquare size={14} style={{ verticalAlign: "middle", marginRight: 6 }} />{t("imap_compose_title")}</div>
          <button onClick={onClose} className="modal-close" aria-label="Close"><X size={16} /></button>
        </div>
        <div className="flex flex-col gap-3">
          <div className="form-group">
            <label className="form-label">{t("imap_from")}</label>
            <select value={form.smtp_config_id ?? ""} onChange={e => set("smtp_config_id", Number(e.target.value))} className="form-input">
              {smtpConfigs.map(c => (
                <option key={c.id} value={c.id}>{c.label} ({c.login})</option>
              ))}
            </select>
          </div>
          <div className="form-group">
            <label className="form-label">{t("imap_to")}</label>
            <input type="text" value={form.to} onChange={e => set("to", e.target.value)} placeholder="recipient@example.com" className="form-input" />
          </div>
          <div className="form-group">
            <label className="form-label">{t("imap_subject")}</label>
            <input type="text" value={form.subject} onChange={e => set("subject", e.target.value)} className="form-input" />
          </div>
          <div className="form-group">
            <label className="form-label">{t("imap_body")}</label>
            <textarea value={form.body} onChange={e => set("body", e.target.value)} rows={8} className="form-input" style={{ resize: "vertical", fontFamily: "inherit" }} />
          </div>
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          <div className="flex-1" />
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
      ...
      <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--border)", background: "var(--card)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3, fontSize: 12, color: "var(--muted)" }}>
          <div><span style={{ color: "var(--dim)" }}>From:</span> {message.from_email}</div>
          {message.to_email && <div><span style={{ color: "var(--dim)" }}>To:</span> {message.to_email}</div>}
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              {message.extracted_order_number && (
                <span style={{ fontSize: 11, color: "var(--blue-t)" }}>
                  <Package size={10} style={{ display: "inline", marginRight: 2 }} />#{message.extracted_order_number}
                </span>
              )}
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "0" }}>
        {message.body ? (
          isHtml ? (
            ...
            <pre style={{ padding: "16px", fontSize: 13, whiteSpace: "pre-wrap", wordBreak: "break-word", margin: 0, color: "var(--text)", fontFamily: "inherit" }}>
              {message.body}
            </pre>
          )
        ) : (
...
function FolderRow({ acc, folder, s, selectedAccount, selectedFolder, onSelectFolder }) {
  ...
  return (
    <div
      ...
      style={{
        padding: "6px 10px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontSize: 12,
        background: isActive ? "rgba(59,130,246,0.1)" : "transparent",
        borderRadius: 4, margin: "1px 4px",
      }}
    >
      <span style={{ color: "var(--muted)", display: "flex" }}><FolderIcon name={folder} size={12} /></span>
      <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{folder}</span>
      {unread > 0 && (
        <span style={{ fontSize: 10, background: "rgba(239,68,68,0.15)", color: "#f87171", borderRadius: 999, padding: "1px 5px" }}>{unread}</span>
      )}
    </div>
  );
}
...
// ─── FolderTree ───────────────────────────────────────────────────────────────
function FolderTree({ accounts, expandedAccounts, accountFolders, loadingFolders, stats,
  selectedAccount, selectedFolder, onToggleExpand, onSelectFolder, onAddImap }) {
  const allUnread = Object.values(stats).reduce((s, a) => s + (a?.unread ?? 0), 0);
  return (
    <div style={{ width: 220, flexShrink: 0, borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", overflowY: "auto", background: "var(--surface)" }}>
      <div style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button onClick={onAddImap} className="btn btn-ghost btn-sm" title="Add IMAP account" style={{ padding: "2px 6px" }}>
          <Plus size={13} />
        </button>
      </div>
      ...
      {accounts.length > 1 && (
        <div
          ...
          style={{
            ...
            background: selectedAccount?.id === -1 ? "rgba(59,130,246,0.08)" : "transparent",
            borderLeft: selectedAccount?.id === -1 ? "2px solid var(--blue)" : "2px solid transparent",
        ...
        >
          ...
          {allUnread > 0 && (
            <span style={{ fontSize: 10, background: "#ef4444", color: "#fff", borderRadius: 999, padding: "1px 5px", flexShrink: 0 }}>{allUnread}</span>
          )}
        </div>
      )}
      {accounts.length === 0 && (
        <div style={{ padding: "20px 12px", textAlign: "center", fontSize: 12, color: "var(--muted)" }}>
          No accounts<br />
        ...
        return (
          <div key={acc.id}>
            ...
            >
              ...
              <Inbox size={13} style={{ color: acc.is_active ? "var(--blue)" : "var(--muted)", flexShrink: 0 }} />
              <span style={{ fontSize: 12, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{acc.label}</span>
              {unread > 0 && (
                <span style={{ fontSize: 10, background: "#ef4444", color: "#fff", borderRadius: 999, padding: "1px 5px", flexShrink: 0 }}>{unread}</span>
              )}
            </div>
            {expanded && (
              <div style={{ paddingLeft: 8 }}>
                {loadingFolders[acc.id] ? (
                  <div style={{ padding: "6px 12px", fontSize: 11, color: "var(--muted)" }}>Loading…</div>
                ) : folders.length === 0 ? (
                  ["INBOX"].map(f => (
                    <FolderRow key={f} acc={acc} folder={f} s={s}
                      selectedAccount={selectedAccount} selectedFolder={selectedFolder} onSelectFolder={onSelectFolder} />
                  ))
                ) : (
                  folders.map(f => <FolderRow key={f} acc={acc} folder={f} s={s}
                    selectedAccount={selectedAccount} selectedFolder={selectedFolder} onSelectFolder={onSelectFolder} />)
                )}
              </div>
            )}
...
function MessageList({ messages, selectedMessage, selectedFolder, selectedAccount,
  msgTotal, msgPage, loadingMsgs, onLoadMessages, onSelectMessage, onSearch,
  onMarkRead, onArchive, onDelete, msgSearch, onMsgContextMenu }) {
  ...
  return (
    <div style={{ width: 300, flexShrink: 0, borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", overflowY: "auto" }}>
      <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--border)", background: "var(--card)", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <button onClick={() => selectedAccount && onLoadMessages(selectedAccount.id, selectedFolder, msgPage, msgSearch)}
          className="btn btn-ghost btn-sm" style={{ padding: "2px 4px" }} title="Refresh">
          <RefreshCw size={11} />
        </button>
      </div>
      <div style={{ padding: "6px 10px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 6, background: "var(--surface)" }}>
      ...
      {loadingMsgs ? (
        <div style={{ padding: 12 }}>
          {[...Array(5)].map((_, i) => (
            <div key={i} style={{ height: 52, background: "rgba(255,255,255,0.03)", borderRadius: 6, marginBottom: 6 }} />
          ))}
        </div>
      ) : messages.length === 0 ? (
        <div style={{ padding: "40px 16px", textAlign: "center", fontSize: 12, color: "var(--muted)" }}>
          <Mail size={30} style={{ opacity: 0.3, marginBottom: 8 }} />
          <div>No messages in {selectedFolder}</div>
          <div style={{ fontSize: 11, marginTop: 4, color: "var(--dim)" }}>Click Check Now to fetch</div>
        </div>
      ) : (
        <div style={{ flex: 1 }}>
          {messages.map(msg => (
            <div
              ...
              style={{
                ...
                background: selectedMessage?.id === msg.id ? "rgba(59,130,246,0.08)" : "transparent",
                borderLeft: selectedMessage?.id === msg.id ? "2px solid var(--blue)" : "2px solid transparent",
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 4 }}>
                <div style={{ fontSize: 12, fontWeight: msg.is_read ? 400 : 600, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              ...
              <div style={{ fontSize: 12, color: msg.is_read ? "var(--muted)" : "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginTop: 2 }}>
              ...
              {(msg.action_taken || msg.extracted_order_number) && (
                <div style={{ display: "flex", gap: 4, marginTop: 3, flexWrap: "wrap" }}>
                  ...
                  {msg.extracted_order_number && (
                    <span style={{ fontSize: 10, color: "var(--blue-t)" }}>#{msg.extracted_order_number}</span>
                  )}
                </div>
              )}
            </div>
          ...
          {totalPages > 1 && (
            <div style={{ display: "flex", justifyContent: "center", gap: 8, padding: 12 }}>
              <button disabled={msgPage <= 1}
                onClick={() => onLoadMessages(selectedAccount.id, selectedFolder, msgPage - 1, msgSearch)}
                className="btn btn-ghost btn-sm">← Prev</button>
              <span style={{ fontSize: 12, color: "var(--muted)", alignSelf: "center" }}>
                {msgPage} / {totalPages}
              </span>
              <button disabled={msgPage >= totalPages}
                onClick={() => onLoadMessages(selectedAccount.id, selectedFolder, msgPage + 1, msgSearch)}
...
// ─── Accounts & SMTP Panel ───────────────────────────────────────────────────
function AccountsPanel({ accounts, smtpConfigs, stats, onAddImap, onEditImap, onDeleteImap, onToggleImap, onAddSmtp, onDeleteSmtp, onTestSmtp, onLinkAll, onClose }) {
  const { t } = useLang();
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-lg)", maxHeight: "80vh", display: "flex", flexDirection: "column" }} role="dialog" aria-modal="true" aria-labelledby="imap-settings-title">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div id="imap-settings-title" style={{ display: "flex", gap: 0 }}>
            {[{ key: "imap", label: "IMAP Accounts" }, { key: "smtp", label: "SMTP Configs" }].map(({ key, label }) => (
              <button key={key} onClick={() => setView(key)} style={{
                padding: "6px 14px", border: "none", background: "transparent", cursor: "pointer", fontSize: 12,
          ...
          {view === "imap" && (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <button onClick={onLinkAll} className="btn btn-b btn-sm" title="Auto-link email pool entries to IMAP accounts by matching login">
                  <Database size={13} /> Auto-link Emails
                </button>
                <button onClick={onAddImap} className="btn btn-g btn-sm"><Plus size={13} /> {t("imap_add_account_btn")}</button>
              </div>
              {accounts.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)", fontSize: 13 }}>{t("imap_no_accounts")}</div>
              ) : (
                <table className="tbl">
                  <thead><tr><th>{t("col_label")}</th><th>{t("col_server")}</th><th>{t("col_status")}</th><th>{t("imap_unread")}</th><th></th></tr></thead>
                  <tbody>
                    {accounts.map(acc => {
                      const s = stats[acc.id];
                      return (
                        <tr key={acc.id}>
                          ...
                          <td style={{ fontFamily: "monospace", fontSize: 11 }}>{acc.host}:{acc.port}</td>
                          <td>
                          ...
                          <td>
                            <div className="tbl-actions">
                              <button onClick={() => onToggleImap(acc)} className="btn btn-ghost btn-sm" title={acc.is_active ? "Pause" : "Resume"}>
                                {acc.is_active ? <ToggleRight size={15} style={{ color: "var(--accent)" }} /> : <ToggleLeft size={15} />}
                              </button>
                              <button onClick={() => { onEditImap(acc); onClose(); }} className="btn btn-ghost btn-sm">{t("btn_edit")}</button>
                              <button onClick={() => onDeleteImap(acc)} className="btn btn-r btn-sm">{t("btn_delete")}</button>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          {view === "smtp" && (
            <>
              <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
                <button onClick={onAddSmtp} className="btn btn-g btn-sm"><Plus size={13} /> {t("imap_add_smtp_btn")}</button>
              </div>
              {smtpConfigs.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)", fontSize: 13 }}>{t("imap_no_smtp")}</div>
              ) : (
                <table className="tbl">
                  <thead><tr><th>{t("col_label")}</th><th>{t("col_host_port")}</th><th>{t("imap_login_label")}</th><th>TLS</th><th></th></tr></thead>
                  <tbody>
                    {smtpConfigs.map(c => (
                      <tr key={c.id}>
                        <td>{c.label}</td>
                        ...
                        <td><span className="st">{c.use_tls ? "TLS" : c.use_starttls ? "STARTTLS" : "None"}</span></td>
                        <td>
                          <div className="tbl-actions">
                            <button onClick={() => onTestSmtp(c.id)} className="btn btn-b btn-sm">{t("btn_test")}</button>
                            <button onClick={() => onDeleteSmtp(c.id)} className="btn btn-r btn-sm">{t("btn_delete")}</button>
                          </div>
                        </td>
                      </tr>
                    ))}
...
// ─── Main Page ───────────────────────────────────────────────────────────────
export default function Imap({ onNavigate }) {
  const { success: toastOk, error: toastErr } = useToast();
  ...
  const loadFolders = useCallback(async (account) => {
    ...
    try {
      ...
      setAccountFolders(p => ({ ...p, [account.id]: ["INBOX"] }));
    } finally {
      setLoadingFolders(p => ({ ...p, [account.id]: false }));
    }
  }, [accountFolders]);

  const loadMessages = useCallback(async (accountId, folder, page, search = "") => {
    if (!accountId) return;
    setLoadingMsgs(true);
    ...
    try {
      ...
      const res = await invoke("get_folder_messages", { accountId, folder, page, search: search || null });
      setMessages(res.items ?? []);
      setMsgTotal(res.total ?? 0);
      setMsgPage(page);
  ...
  const loadSentEmails = useCallback(async (page) => {
    setLoadingSent(true);
    try {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      u1 = await listen("imap_check_progress", (event) => {
        const { account_id, messages, error } = event.payload;
        if (error) {
          ...
          setSelectedAccount(prev => {
            ...
            if (prev.id === -1) {
              ...
              invoke("get_unified_inbox", { page: 1, search: null })
                .then(res => { setMessages(res.items ?? []); setMsgTotal(res.total ?? 0); }).catch(() => {});
            } else if (prev.id === account_id) {
              invoke("get_folder_messages", { accountId: account_id, folder, page: 1, search: null })
                .then(res => { setMessages(res.items ?? []); setMsgTotal(res.total ?? 0); }).catch(() => {});
            }
            return prev;
          });
  ...
  const handleCheckAll = async () => {
    ...
    try {
      ...
      const pending = {};
      accounts.filter(a => a.is_active).forEach(a => { pending[a.id] = true; });
  ...
  const handleDelete = async (acc) => {
    const ok = await confirm(`${t("imap_confirm_delete")} "${acc.label}"?`, { title: t("imap_delete_account") });
    if (!ok) return;
    try {
    ...
    <div className="content" style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div className="ph">
        <div>
          ...
          <div className="ph-sub">{accounts.length} accounts · {smtpConfigs.length} SMTP</div>
        </div>
        <div className="ph-actions">
          <div style={{ display: "flex", border: "1px solid var(--border)", borderRadius: 6, overflow: "hidden" }}>
            <button onClick={() => setViewMode("inbox")} className="btn btn-sm" style={{ borderRadius: 0, background: viewMode === "inbox" ? "var(--blue)" : "transparent", color: viewMode === "inbox" ? "#fff" : "var(--muted)", border: "none" }}>
              <Inbox size={13} /> Inbox
            </button>
            <button onClick={() => setViewMode("sent")} className="btn btn-sm" style={{ borderRadius: 0, background: viewMode === "sent" ? "var(--blue)" : "transparent", color: viewMode === "sent" ? "#fff" : "var(--muted)", border: "none" }}>
              <Send size={13} /> Sent
            </button>
          </div>
          <button onClick={() => setShowCompose(true)} className="btn btn-g btn-sm" disabled={smtpConfigs.length === 0} title={smtpConfigs.length === 0 ? "Add SMTP config first" : "Compose"}>
          ...
          <button onClick={() => setShowAccountsPanel(true)} className="btn btn-ghost btn-sm" title="Manage accounts & SMTP">
            <Settings size={13} />
          </button>
        </div>
      </div>
      ...
      {viewMode === "inbox" && (
        <div style={{ flex: 1, display: "flex", minHeight: 0, overflow: "hidden" }}>
          ...
          <MessageList
            ...
            onSearch={(val) => { setMsgSearch(val); loadMessages(selectedAccount?.id, selectedFolder, 1, val); }}
      ...
      {viewMode === "sent" && (
        <div style={{ flex: 1, overflowY: "auto", padding: "0 4px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 4px" }}>
            <span style={{ fontSize: 12, color: "var(--muted)" }}>{sentTotal} sent email{sentTotal !== 1 ? "s" : ""}</span>
            <button onClick={() => loadSentEmails(sentPage)} className="btn btn-ghost btn-sm"><RefreshCw size={13} /></button>
          </div>
          {loadingSent ? (
            <div style={{ padding: "40px 0", textAlign: "center", color: "var(--muted)", fontSize: 13 }}>Loading…</div>
          ) : sentEmails.length === 0 ? (
            <div style={{ padding: "60px 0", textAlign: "center", color: "var(--muted)", fontSize: 13 }}>No sent emails</div>
          ) : (
            <table className="tbl">
              <thead>
                <tr>
              ...
              <tbody>
                {sentEmails.map(e => (
                  <tr key={e.id}>
                    ...
                    <td>
                      <span className={`st ${e.status === "sent" ? "st-delivered" : e.status === "failed" ? "st-decline" : "st-pending"}`}>
                        {e.status}
                      </span>
                    </td>
                    <td style={{ fontSize: 11, color: "var(--muted)" }}>{e.sent_at ? e.sent_at.slice(0, 16).replace("T", " ") : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {sentTotal > 50 && (
            <div style={{ display: "flex", justifyContent: "center", gap: 8, padding: "12px 0" }}>
              <button disabled={sentPage <= 1} onClick={() => loadSentEmails(sentPage - 1)} className="btn btn-ghost btn-sm">← Prev</button>
              <span style={{ fontSize: 12, color: "var(--muted)", lineHeight: "28px" }}>Page {sentPage} / {Math.ceil(sentTotal / 50)}</span>
              <button disabled={sentPage >= Math.ceil(sentTotal / 50)} onClick={() => loadSentEmails(sentPage + 1)} className="btn btn-ghost btn-sm">Next →</button>
            </div>
          )}
        </div>
      )}
      ...
      {contextMenu && (
        <div
          style={{
            ...
            boxShadow: "0 4px 20px rgba(0,0,0,0.3)", zIndex: 9999, minWidth: 160, padding: "4px 0",
          }}
          onClick={(e) => e.stopPropagation()}
        >
          {[
            { label: "Mark as Read", icon: <CheckCircle size={12} />, action: () => { handleMarkRead(contextMenu.msg); setContextMenu(null); } },
            { label: "Archive", icon: <Archive size={12} />, action: () => { handleArchiveMessage(contextMenu.msg); setContextMenu(null); } },
            { label: "Delete", icon: <Trash2 size={12} />, action: () => { handleDeleteMessage(contextMenu.msg); setContextMenu(null); } },
      ...
      {/* ── Modals ── */}
      {showAddImap && <AccountModal account={null} onSave={handleSaveImapAccount} onClose={() => setShowAddImap(false)} />}
      {editAccount && <AccountModal account={editAccount} onSave={handleSaveImapAccount} onClose={() => setEditAccount(null)} />}
      {showAddSmtp && <SmtpModal onSave={handleSaveSmtp} onClose={() => setShowAddSmtp(false)} />}
      {showCompose && <ComposeModal smtpConfigs={smtpConfigs} defaultTo={composeReply?.to} defaultSubject={composeReply?.subject} defaultBody={composeReply?.body} onSent={() => { setShowCompose(false); setComposeReply(null); }} onClose={() => { setShowCompose(false); setComposeReply(null); }} />}
      {showAccountsPanel && (
        <AccountsPanel
          accounts={accounts}
          smtpConfigs={smtpConfigs}
          stats={stats}
          onAddImap={() => { setShowAccountsPanel(false); setShowAddImap(true); }}
          onEditImap={(acc) => setEditAccount(acc)}
          onDeleteImap={handleDelete}
          onToggleImap={handleToggle}
          onAddSmtp={() => { setShowAccountsPanel(false); setShowAddSmtp(true); }}
