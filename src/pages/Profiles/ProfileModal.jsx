...
export function ProfileModal({ onCreated, onClose }) {
  ...
  useEffect(() => {
    ...
    invoke("get_cards", {
      filter: { status: "free", search: null, country: null, bank_name: null, source: null, date_from: null, date_to: null },
      page: 1,
      perPage: 200,
    })
    ...
    invoke("get_profile_templates").then(setTemplates).catch(() => {});
  ...
  const handleCreate = async () => {
    ...
    if (!id) { toast("Select a free card", "warn"); return; }
    setLoading(true);
    try {
      const p = await invoke("create_profile", { cardId: id, notes: mode === "quick" ? null : (notes || null) });
      ...
      if (mode === "quick" && autoCreateDrop) {
        ...
        } catch { /* billing reveal failed — profile still created */ }
      }
      if (saveAsTemplate && templateName.trim()) {
        ...
        await invoke("save_profile_template", {
          ...
          state: null, city: null, phonePrefix: null, source: null,
        }).catch(() => {});
      }
      ...
      if (emailId) {
        ...
        } catch (_) { /* email linking failed — profile still created */ }
      }
  ...
  return (
    <div className="modal-overlay">
      <div ref={createProfRef} className="modal w-[480px]" role="dialog" aria-modal="true" aria-labelledby="create-profile-title" >
        <div style={{
          ...
          <div className="flex items-center gap-3">
            ...
            <span id="create-profile-title" className="modal-title m-0">{t("new_profile")}</span>
            <div className="flex gap-1" style={{ background: "var(--surface)", borderRadius: 6, padding: 2 }}>
              {["quick", "full"].map((m) => (
                <button
                  key={m}
                  onClick={() => setMode(m)}
        ...
        <div className="p-6 flex flex-col gap-4">
              ...
              {cardsLoading ? (
                ...
                <div
                  ...
                  style={{
                    ...
                    background: cardId === String(c.id) ? "rgba(96,165,250,0.12)" : "transparent",
                    borderBottom: "1px solid var(--border)",
                    transition: "background 0.15s",
                  }}
                  onMouseEnter={(e) => { if (cardId !== String(c.id)) e.currentTarget.style.background = "rgba(255,255,255,0.03)"; }}
                  onMouseLeave={(e) => { if (cardId !== String(c.id)) e.currentTarget.style.background = "transparent"; }}
                >
                  <div className="flex items-center gap-2">
                    {c.card_type && <span className="text-[10px] text-muted uppercase font-semibold">{c.card_type}</span>}
                    <span className="font-mono text-[12px] text-text">
                      ●●●● {c.last4 || "????"}
                    </span>
                    {c.expiry_date && <span className="text-[11px] text-muted">{c.expiry_date}</span>}
                  </div>
                  <div className="flex gap-2 items-center">
                    {c.bank_name && <span className="text-[11px] text-muted">{c.bank_name}</span>}
                    {c.country && <span className="text-[10px] font-mono text-muted">{c.country}</span>}
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="form-label" style={{ marginBottom: 4 }}>Email (optional)</label>
            ...
            <datalist id="email-suggestions">
              {availableEmails.map(e => <option key={e.id} value={e.email} />)}
            </datalist>
          </div>

          {mode === "quick" ? (
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--muted)", cursor: "pointer" }}>
              <input type="checkbox" checked={autoCreateDrop} onChange={(e) => setAutoCreateDrop(e.target.checked)} />
              ...
              {templates.length > 0 && (
                <div className="form-group">
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    {templates.map((tpl) => (
                      <div key={tpl.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "var(--surface2)", borderRadius: 6, padding: "4px 8px" }}>
                        <button
                          type="button"
                          style={{ flex: 1, textAlign: "left", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "var(--fg)" }}
                          onClick={() => { if (tpl.source) setNotes((n) => n ? n : `Source: ${tpl.source}`); }}
                          title="Apply template"
                        >
                          {tpl.name}{tpl.country ? ` · ${tpl.country}` : ""}
                        </button>
                        <button
                          type="button"
                          className="btn btn-ghost btn-sm"
