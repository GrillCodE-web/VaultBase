...
export function ProfileRow({
  ...
  return (
    <tr
      ...
      style={{
        ...
        background: isSelected
          ...
          : !hasDrops ? "rgba(234,179,8,0.04)" : undefined,
        outline: isSelected ? "1px solid rgba(96,165,250,0.3)" : undefined,
    ...
    >
      ...
      <td><span className={`st ${cardStatusCss}`}>{rawStatus}</span></td>
      ...
      <td className="text-[11px] text-muted max-w-[110px] overflow-hidden text-ellipsis whitespace-nowrap" title={p.notes ?? ""}>{p.notes || "—"}</td>
      <td className="text-[11px] text-muted whitespace-nowrap">{p.created_at?.slice(0,10)}</td>
      <td onClick={(e) => e.stopPropagation()}>
        <div className="tbl-actions">
          <button className="btn btn-b btn-sm" title={t("float_window")} onClick={() => invoke("open_float_window", { profileId: p.id }).catch(() => {})}>{t("float_window")}</button>
          <button
          ...
          <ActionsMenu items={[
            ...
            { divider: true },
            { label: t("profile_duplicated").split(" ")[0] || "Duplicate", icon: Layers, onClick: onDuplicate },
            { divider: true },
