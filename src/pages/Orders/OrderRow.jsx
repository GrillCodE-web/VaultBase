...
function NeedsAttentionBadge({ order }) {
  // eslint-disable-next-line react-hooks/rules-of-hooks -- useMemo for stable time calculation
  const now = useMemo(() => Date.now(), []);
  const daysSinceCreated = (now - new Date(order.created_at).getTime()) / (1000 * 60 * 60 * 24);
  const daysSinceUpdated = (now - new Date(order.updated_at ?? order.created_at).getTime()) / (1000 * 60 * 60 * 24);

  if (order.status === "pending" && daysSinceCreated > 3) {
    return (
      <AlertTriangle
        size={13}
        style={{ color: "#f97316", flexShrink: 0, marginLeft: 4 }}
  ...
  if (order.status === "shipped" && daysSinceUpdated > 14) {
    return (
      <AlertTriangle
        size={13}
        style={{ color: "#ef4444", flexShrink: 0, marginLeft: 4 }}
...
export function OrderRow({
  ...
  const getBorderColor = () => {
    ...
    if (order.status === "declined" || order.status === "cancelled") return `2px solid ${STATUS_COLORS.error}61`;
    if (order.status === "pending") return `2px solid ${STATUS_COLORS.warning}61`;
    if (order.status === "shipped" || order.status === "in_transit") return `2px solid ${STATUS_COLORS.info}61`;
  ...
  return (
    <>
      ...
      >
        ...
        <td style={{ color: STATUS_COLORS.info, fontFamily: "'JetBrains Mono',monospace", textAlign: "right", whiteSpace: "nowrap" }}>
        ...
        <td style={{ fontSize: 11, color: "var(--muted)", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={order.notes ?? ""}>{order.notes ?? "—"}</td>
          ...
          <div className="tbl-actions">
            ...
            {order.profile_id && (
              <button
                ...
                onClick={(e) => { e.stopPropagation(); invoke("open_float_window", { profileId: order.profile_id }).catch(() => {}); }}
              >⬡</button>
            )}
            <button className="btn btn-r btn-sm" onClick={(e) => { e.stopPropagation(); onDelete(); }}>Del</button>
          </div>
        </td>
      </tr>
      {isExpanded && (
        <tr key={`${order.id}-timeline`}>
          <td colSpan={13} style={{ padding: "12px 16px 16px", background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
            {TimelineComponent}
          </td>
        </tr>
      )}
