...
export function BatchImportModal({ onCreated, onClose }) {
  ...
  return (
    <div className="modal-overlay">
      <div ref={modalRef} className="modal" style={{ width: "var(--modal-sm)" }} role="dialog" aria-modal="true" aria-labelledby="batch-import-title">
        ...
        {!result ? (
          <div className="flex flex-col gap-4">
            <div style={{
              ...
              <div style={{ fontWeight: 600, marginBottom: 4, color: "var(--text-2)" }}>CSV format:</div>
            ...
            {parsedRows.length > 0 && (
              <div style={{
                ...
                <span style={{ color: STATUS_COLORS.info, fontWeight: 600 }}>{parsedRows.length}</span>
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3">
              <div style={{ background: STATUS_COLORS.successBg, border: `1px solid ${STATUS_COLORS.success}33`, borderRadius: 10, padding: 16, textAlign: "center" }}>
                <div style={{ fontSize: 28, fontWeight: 700, color: STATUS_COLORS.success }}>{result.created}</div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>Created</div>
              </div>
              <div style={{ background: STATUS_COLORS.errorBg, border: `1px solid ${STATUS_COLORS.error}33`, borderRadius: 10, padding: 16, textAlign: "center" }}>
                <div style={{ fontSize: 28, fontWeight: 700, color: STATUS_COLORS.error }}>{result.failed}</div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>Failed</div>
              </div>
            </div>
            <button onClick={onClose} className="btn btn-g w-full">Done</button>
          </div>
        )}
      </div>
    </div>
