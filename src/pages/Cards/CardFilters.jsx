    ...
    <>
      ...
      <div className="filters">
        ...
        >
          ...
          {filterMeta.banks.map(b => <option key={b} value={b}>{b}</option>)}
        </select>
        ...
        >
          ...
          {filterMeta.sources.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        ...
        <input
          ...
          placeholder="ZIP" title={t("cards_zip_filter_hint")}
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <input
            ...
            style={{ width: 200 }}
          />
          <button onClick={handleSearch} className="btn btn-b btn-sm">{t("btn_search")}</button>
          <button onClick={handleResetFilters} className="btn btn-ghost btn-sm">{t("cc_reset_filters")}</button>
          <button onClick={loadCards} className="btn btn-ghost btn-sm" title="Refresh" data-shortcut="refresh">{loading ? "⟳" : "↺"}</button>
        </div>
      </div>
        ...
        <div className="flex gap-1.5 mb-2.5 flex-wrap">
          {filter.bank_name && (
            <span style={{
              ...
              color: "#60a5fa", fontSize: 11, display: "inline-flex", alignItems: "center", gap: 6,
            }}>
