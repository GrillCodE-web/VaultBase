...
export function CardTimelinePanel({ cardId, onClose }) {
  ...
  useEffect(() => {
    invoke("get_card_timeline", { cardId }).then(setEvents).catch(() => setEvents([]));
  }, [cardId]);
  return (
    <div style={{
        ...
        <div className="relative" style={{ paddingLeft: 20 }}>
          ...
          {events.map((ev, i) => (
            <div key={i} className="relative mb-3" style={{ paddingLeft: 12 }}>
              <div style={{
                position: "absolute", left: -7, top: 4, width: 8, height: 8,
                borderRadius: "50%",
                background: ev.event_type === "order" ? "var(--blue)" : ev.event_type.includes("card") ? "var(--accent)" : "var(--border)",
                border: "1.5px solid var(--surface)",
              }} />
              <div className="text-[11px] text-muted">{ev.created_at.slice(0, 16).replace("T", " ")}</div>
