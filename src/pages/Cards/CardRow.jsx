...
export function CardRow({
  ...
  return (
    <tr
      ...
      style={{
        background: selected.has(card.id) ? "rgba(59,130,246,0.06)" : undefined,
        cursor: "pointer",
    ...
    >
      ...
      {visibleCols.includes("card_number") && (
        <td
          onClick={e => { e.stopPropagation(); handleCopyToast(displayNum); }}
          title={t("btn_copy")}
      ...
      {visibleCols.includes("expiry") && (
        <td><ExpiryCell expiry={card.expiry_date} t={t} /></td>
      )}

      {/* CVV */}
      {visibleCols.includes("cvv") && (
        <td
          style={{ fontSize: 11, color: "var(--text-2)", fontFamily: "'JetBrains Mono',monospace", cursor: rev?.cvv ? "copy" : undefined }}
          onClick={e => { if (rev?.cvv) { e.stopPropagation(); handleCopyToast(rev.cvv); } }}
      ...
      {visibleCols.includes("holder") && (
        <td
          style={{ color: "var(--text-2)", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", cursor: "copy" }}
          onClick={e => { e.stopPropagation(); handleCopyToast(rev?.holder_name || card.holder_name || ""); }}
          title={t("btn_copy")}
        >
      ...
      {visibleCols.includes("billing") && (
        <td
          style={{ fontSize: 11, color: "var(--text-2)", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", cursor: rev?.billing_address ? "copy" : undefined }}
          onClick={e => { if (rev?.billing_address) { e.stopPropagation(); handleCopyToast(rev.billing_address); } }}
      ...
      {visibleCols.includes("zip") && (
        <td
          style={{ fontSize: 11, color: "var(--text-2)", fontFamily: "'JetBrains Mono',monospace", cursor: card.zip ? "copy" : undefined }}
          onClick={e => { if (card.zip) { e.stopPropagation(); handleCopyToast(card.zip); } }}
      ...
      {visibleCols.includes("city") && (
        <td
          ...
          onClick={e => { if (card.city) { e.stopPropagation(); handleCopyToast(card.city); } }}
      ...
      {visibleCols.includes("state") && (
        <td
          ...
          onClick={e => { if (card.state) { e.stopPropagation(); handleCopyToast(card.state); } }}
      ...
      {visibleCols.includes("country") && (
        <td>
          {card.country
            ? (
              <span
                onClick={e => { e.stopPropagation(); setFilter(f => ({ ...f, country: card.country })); setPage(1); }}
                style={{ padding: "2px 6px", background: "var(--surface)", borderRadius: 5, fontSize: 11, color: "var(--text-2)", cursor: "pointer", display: "inline-block" }}
      ...
      {visibleCols.includes("phone") && (
        <td
          style={{ fontSize: 11, color: "var(--text-2)", fontFamily: "'JetBrains Mono',monospace", cursor: rev?.phone ? "copy" : undefined }}
          onClick={e => { if (rev?.phone) { e.stopPropagation(); handleCopyToast(rev.phone); } }}
      ...
      {visibleCols.includes("bin_bank") && (
        <td>
          ...
          {card.bank_name && (
            <span
              onClick={e => { e.stopPropagation(); setFilter(f => ({ ...f, bank_name: card.bank_name })); setPage(1); }}
              style={{ display: "block", fontSize: 11, color: "var(--muted)", cursor: "pointer", textDecoration: "underline dotted" }}
          ...
          <div className="flex items-center gap-1">
            <div className="status-menu-anchor relative">
              <span
                ...
                onClick={e => { e.stopPropagation(); setStatusMenuId(statusMenuId === card.id ? null : card.id); }}
              ...
              {statusMenuId === card.id && (
                <div style={{
                  ...
                  ].filter(o => o.s !== card.status).map(o => (
                    <button
                      key={o.s}
                      onClick={e => { e.stopPropagation(); setStatusMenuId(null); handleStatusChange(card.id, o.s); }}
                      style={{
            ...
            {burnCount > 0 && (
              <span
                style={{
                  ...
                  color: burnCount >= 5 ? "#fb923c" : burnCount >= 3 ? "#facc15" : "var(--muted)",
                }}
      ...
      {visibleCols.includes("health") && (
        <td onClick={e => e.stopPropagation()}>
          {(() => {
            ...
            if (!h) return <span className="text-muted" style={{ fontSize: 11 }}>—</span>;
            return (
      ...
      {visibleCols.includes("ip") && (
        <td style={{ fontSize: 11, color: "var(--text-2)", fontFamily: "'JetBrains Mono',monospace" }}>{rev?.ip_address || "—"}</td>
      )}

      {/* Actions */}
      <td onClick={e => e.stopPropagation()}>
        <div className="tbl-actions">
          <button onClick={() => setShopUsageCardId(card.id)} className="btn btn-ghost btn-sm" title="Shops used" style={{ padding: "3px 7px" }}>
            <Store size={12} />
          </button>
          <button onClick={() => setTimelineCardId(card.id)} className="btn btn-ghost btn-sm" title="Timeline" style={{ padding: "3px 7px" }}>
            <Clock size={12} />
          </button>
          <ActionsMenu items={[
            {
              label: t("cc_copy_num"), icon: Copy,
              onClick: () => {
                const num = rev?.card_number ? formatCardNumber(rev.card_number) : formatBinMasked(card.bin, card.last4);
                handleCopyToast(num);
              },
            },
            { label: t("cards_copy_full"), icon: Copy, onClick: () => handleCopyToast(buildPipeString(card, rev)) },
            { divider: true },
            { label: t("cc_mark_free"), icon: CheckCircle, onClick: () => handleStatusChange(card.id, "free") },
            { label: t("cc_mark_dead"), icon: XCircle, onClick: () => handleStatusChange(card.id, "dead"), danger: true },
            { divider: true },
            {
              label: t("new_profile"), icon: User,
              onClick: async () => {
                try {
            ...
            { divider: true },
            { label: t("btn_delete"), icon: Trash2, onClick: () => handleDelete(card.id), danger: true },
          ]} />
        </div>
      </td>
    </tr>
