...
export function ImportModal({ onClose, onImported, initialRaw = '' }) {
  ...
  // REDESIGN-05-2: scroll-lock/Escape/focus-trap теперь внутри <Modal> (BUG-012 сохранён)
  ...
  // REDESIGN-05-2: ручной оверлей/шапка/футер/focus-trap/Escape
    ...
    <Modal
      ...
      footer={
        <>
          ...
          {step === 1 && (
            <button onClick={handlePreview} disabled={loading || !raw.trim()} className="btn btn-b">
              {loading && <Spinner />}
              {t('cc_import_preview')} →
            </button>
          )}
                      ...
                      <th key={i}>
                        <span
                          style={{
                            ...
                            border: field !== 'skip' ? '1px solid var(--color-success-bg)' : 'none',
                          }}
                        >
