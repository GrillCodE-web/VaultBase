...
function UpdateCard({ item, onApplyTrack, onIgnore }) {
  ...
  const LABELS = {
    track: "upd_type_track", delivered: "upd_type_delivered",
    confirmed: "upd_type_confirmed", cancelled: "upd_type_cancelled",
    ...
    <div className="upd-card">
      <div className="upd-head">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[11px] text-muted mt-0.5">
            {label}{item.entity_type ? ` · ${item.entity_type}` : ""}{item.entity_id ? ` #${item.entity_id}` : ""}
          </div>
        </div>
      ...
      {isTrack && item.tracking_number && (
        <div className="trk-box">
          ...
          <div className="trk-acts">
            <button className="btn btn-g btn-sm" onClick={() => onApplyTrack?.(item)}>✓ {t("upd_apply_track")}</button>
            <button className="btn btn-ghost btn-sm" onClick={() => onIgnore?.(item)}>{t("upd_ignore")}</button>
          </div>
        </div>
      )}
      ...
      {typeKey === "cancelled" && (
        <div className="cancel-box">
          ...
          <div className="flex gap-1">
            <button className="btn btn-o btn-sm btn-icon"><RefreshCw size={12} /> {t("upd_rebid")}</button>
      ...
      {typeKey === "delivered" && (
        <div className="status-row flex items-center gap-2">
          ...
          <button className="btn btn-o btn-sm btn-icon ml-auto"><RefreshCw size={12} /> {t("upd_rebid")}</button>
        </div>
      )}
    </div>
  );
...
export default function Updates() {
  ...
  const filteredItems = activeTab === "all" ? items : items.filter((item) => getUpdateType(item) === activeTab);
  ...
  const bannerBg    = isBeta ? "rgba(234,179,8,0.08)"  : "rgba(239,68,68,0.08)";
  const bannerBdr   = isBeta ? "rgba(234,179,8,0.25)"  : "rgba(239,68,68,0.25)";
    ...
    <div className="content">
      <div className="ph">
        ...
        <div className="ph-actions">
          <button className="btn btn-ghost btn-sm" onClick={handleRefresh} disabled={loading || refreshing}>
            {refreshing ? "…" : <><RefreshCw size={13} /> {t("upd_refresh")}</>}
          </button>
      ...
      {showBanner && (
        <div style={{
          ...
          <div className="flex items-center justify-between" style={{ marginBottom: downloadState === "downloading" ? 8 : 0 }}>
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              ...
              {downloadState === "downloading" && (
                <span className="text-[12px] text-muted">{t("upd_downloading")} {downloadProgress}%</span>
              )}
              {downloadState === "ready" && (
      ...
      {!loading && !error && (
        <div className="sum-bar">
          <div className="sum-item"><div className="sum-num text-green-t">{counts.track}</div><div className="sum-lbl">{t("upd_new_tracks_count")}</div></div>
          <div className="sum-item"><div className="sum-num text-cyan-t">{counts.delivered}</div><div className="sum-lbl">✅ {t("upd_type_delivered")}</div></div>
          <div className="sum-item"><div className="sum-num text-orange-t">{counts.confirmed}</div><div className="sum-lbl">🧾 {t("upd_type_confirmed")}</div></div>
          <div className="sum-item"><div className="sum-num text-red-t">{counts.cancelled}</div><div className="sum-lbl">❌ {t("upd_type_cancelled")}</div></div>
          <div className="sum-item"><div className="sum-num text-yellow-t">{counts.attention}</div><div className="sum-lbl">⚠️ {t("upd_type_attention")}</div></div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-[13px] text-red-t" >
          {error}<br />
          <button className="btn btn-ghost btn-sm btn-icon mt-3" onClick={handleRefresh}><RefreshCw size={13} /> {t("upd_refresh")}</button>
        </div>
