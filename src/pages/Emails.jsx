...
import {
  ...
  RefreshCw, Search, X, Check, Filter, Store, Info, Link2, Unlink
} from "lucide-react";
      ...
      {open && (
        <div style={{
          ...
          {imapAccounts.length === 0 ? (
            <div style={{ padding: "10px 14px", fontSize: 12, color: "var(--muted)", textAlign: "center" }}>
              No IMAP accounts.{" "}
              <button
                onClick={() => { setOpen(false); onNavigate?.("imap"); }}
                style={{ color: "var(--accent)", background: "none", border: "none", cursor: "pointer", fontSize: 12 }}
              >{t("imap_add_account")} →</button>
            </div>
          ) : (
            <>
              <div style={{ padding: "6px 14px", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--muted)", borderBottom: "1px solid var(--border)" }}>
                Select IMAP account
              </div>
              {imapAccounts.map((acc) => (
                <button
                  key={acc.id}
                  onClick={() => { onLink(entry.id, acc.id); setOpen(false); }}
                  style={{ width: "100%", textAlign: "left", padding: "8px 14px", fontSize: 12, color: "var(--text)", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}
                >
                  <MailCheck size={12} style={{ color: "var(--accent)", flexShrink: 0 }} />
                  <span className="overflow-hidden text-ellipsis whitespace-nowrap">{acc.name || acc.login}</span>
                </button>
              ))}
            </>
          )}
...
function EmailModal({ initial, onSave, onClose }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal" style={{ width: "var(--modal-sm)" }} role="dialog" aria-modal="true" aria-labelledby="email-modal-title">
        <div className="flex items-center justify-between mb-[18px]">
          <div id="email-modal-title" className="modal-title">{isEdit ? t("email_modal_title_edit") : t("email_modal_title_add")}</div>
        ...
        <div className="flex flex-col gap-3.5">
          ...
          >
            {loading ? t("email_saving") : isEdit ? t("email_save_changes") : t("email_modal_title_add")}
          </button>
        </div>
      </div>
    </div>
...
function EmailShopsCell({ emailId }) {
  ...
  useEffect(() => {
    invoke("get_email_footprint_stats", { emailId }).then(setStats).catch(() => {});
  }, [emailId]);
  if (!stats) return <span className="text-muted text-[11px]">—</span>;
  return (
    <div className="flex items-center gap-1">
      <span style={{ fontSize: 11, color: stats.unique_shops > 0 ? "var(--text-2)" : "var(--muted)" }}>
        {stats.unique_shops} shop{stats.unique_shops !== 1 ? "s" : ""}
      </span>
      {stats.is_burned && (
        <span style={{
          fontSize: 9, fontWeight: 700, padding: "1px 5px",
          borderRadius: 4, background: STATUS_COLORS.errorBg, color: STATUS_COLORS.error,
        }}>BURNED</span>
      )}
    </div>
  );
...
export default function EmailPool({ onNavigate, inTab = false }) {
  ...
  const load = useCallback(async (p = page, fb = filterBlocked) => {
    setLoading(true);
    try {
      const filter = fb === "used"
        ? { is_blocked: false, is_used: true }
        : { is_blocked: fb };
  ...
  useEffect(() => {
    load();
    invoke("get_imap_accounts").then(setImapAccounts).catch(() => {});
  }, []);
  ...
  const blockedCount = emails.filter((e) => e.is_blocked).length;
  const usedCount    = emails.filter((e) => !e.is_blocked && e.shops_used?.length > 0).length;
    ...
    <>
      ...
      {inTab && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ fontSize: 12, color: "var(--muted)" }}>{cleanCount} clean / {total} total</div>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-g btn-sm" onClick={() => setModal("add")}>+ {t("add_email")}</button>
      ...
      {selected.size > 0 && (
        <div style={{
          ...
          <span style={{ color: STATUS_COLORS.info, fontWeight: 600 }}>{selected.size} selected</span>
          <span style={{ color: "var(--border)", margin: "0 4px" }}>|</span>
          <button className="btn btn-r btn-sm" onClick={handleBulkBlock}><ShieldOff size={12} /> Block Selected</button>
          <button className="btn btn-r btn-sm" onClick={handleBulkDelete}><Trash2 size={12} /> Delete Selected</button>
            ...
            <tr>
              <th className="bg-card" style={{ width: 36 }}>
                <input type="checkbox"
                  ...
                  onChange={e => e.target.checked ? setSelected(new Set(emails.map(em => em.id))) : setSelected(new Set())}
                />
              </th>
          ...
          <tbody>
            ...
            {emails.length === 0 && !loading && (
              <EmptyState
                ...
                action={<button className="btn btn-g btn-sm" onClick={() => setModal("add")}><Plus size={12} /> {t("add_email")}</button>}
              />
            )}
            {emails.map((entry) => (
              <tr key={entry.id} style={{ opacity: entry.is_blocked ? 0.6 : 1, background: selected.has(entry.id) ? STATUS_COLORS.infoBg : undefined }}>
                ...
                <td><span className={`st st-${statusLabel(entry)}`}>{statusLabel(entry)}</span></td>
                ...
                <td>
                  <div className="tbl-actions">
                    <button className="btn btn-ghost btn-sm" onClick={() => setModal(entry)}>{t("btn_edit")}</button>
                    <button
                    ...
                    <button className="btn btn-r btn-sm" onClick={() => handleDelete(entry)}>{t("btn_delete")}</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex gap-1">
            {buildPageNumbers(page, totalPages).map((p) => (
              p === '…' ? (
                <span key={`ellipsis-${p}`} className="px-2 text-muted">…</span>
              ) : (
                <button
                  key={p}
    ...
    <div className="content">
      <div className="ph">
        <div>
          ...
          <div className="ph-sub">{cleanCount} {t("email_clean_subtitle")} / {total} {t("mailboxes_count")}</div>
        </div>
        <div className="ph-actions">
          <button className="btn btn-g" onClick={() => setModal("add")}>+ {t("add_email")}</button>
          <button className="btn btn-b btn-sm" onClick={() => load()}>
            <RefreshCw size={13} style={{ animation: loading ? "spin 1s linear infinite" : "none" }} />
            {t("btn_refresh")}
          </button>
