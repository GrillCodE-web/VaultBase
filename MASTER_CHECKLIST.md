...
> **Всего пунктов:** 199 · **Выполнено:** 146 (из них 9 «уже было») · **Открыто:** 46  
...
| REDESIGN-05-3   | Страницы по одной: Dashboard → Orders → Cards → Profiles → остальные → auth-контур → float-окно                                                                                                                     | 🔴        | `pages/`, `pages.css`, `float.jsx`                                                                   | ⬜                       |
| REDESIGN-05-4   | Фичи frontend-only: порции 1–5, статистика/рекомендатель, заметки, FTS-поиск, виртуализация, колонки, фильтры-пресеты                                                                                               | 🟠        | `src/**`                                                                                             | ⬜                       |
| REDESIGN-05-5   | Фичи, ждущие backend-сессию: пул карт/бронирование, панель воркеров, трекинг-поллер, E2E-чат. Разбито на backend-подпункты 5B1–5B4 (@main); frontend-часть остаётся за @r                                           | 🟠        | стык с backend                                                                                       | 🔄 @main (backend-задел) |
...
| REDESIGN-05-5B2 | Backend: панель воркеров — presence/last_seen/счётчики срезов/групповая статистика (decline-rate) + публикация статов воркером                                                                                      | 🟠        | `cc-sync-server/ws-tauri.js`, `database.js`, `src-tauri/commands/`, `background.rs`                  | ⬜                       |
| REDESIGN-05-5B3 | Backend: трекинг — checkpoints (миграция + поллер), сигнал «перебивай», сессия перебивки (bulk-статусы), правило «delivered >24ч», подсказка привязки трека из почты                                                | 🟠        | `src-tauri/tracking.rs`, `background.rs`, `database/_migrations.rs`, `commands/orders.rs`            | ⬜                       |
