...
| UX-003 | Единые loading/empty/error состояния на ВСЕХ страницах       | 🟡        | Все страницы                | ⬜     |
...
| FEAT-002 | Risk V2: ML-подобный скоринг (магазин, карта, время суток, сумма)                  | 🟡        | `_orders.rs`            | ⬜     |
...
| MGR-007 | Менеджер: движок числовых алертов (правила на расшифрованных отчётах: спайк деклайна, dead-ratio, квоты) + показ в UI; опц. generic webhook. БЕЗ Telegram | 🟠        | `manager-app`                                 | 🔄 @main |
| MGR-008 | Сервер-хард: sha-256-хеш токенов в БД, kill-switch деплоя, анти-replay WS. Бэкапы БД — НЕ делаем (бэкап = вторая копия данных и риск компрометации) | 🟠        | `cc-sync-server`                              | ⬜       |
| MGR-009 | Менеджер: tauri-plugin-updater (отдельный signing key) + staged rollout (stable/beta, %)                                       | 🟡        | `manager-app/src-tauri`                       | ⬜       |
| MGR-010 | Полное управление лицензиями из manager-app: список лицензий, создание (worker/manager), деактивация/бан — админ-эндпоинты на сервере (минимум логики, только CRUD лицензий), UI в manager-app | 🟠        | сервер + manager-app                          | ⬜       |
...
| MGR-013 | Воркер: panic-пароль (duress) — второй пароль на экране входа → криптостирание БД (rekey на случайный ключ) + перезапись/удаление БД, WAL, конфигов и локальных секретов; снаружи выглядит как «неверный пароль»; + удалённый wipe командой менеджера через heartbeat-политику | 🟠        | `src-tauri`, `cc-sync-server`, `manager-app`  | ⬜       |
...
> Подсчитано по фактическим строкам таблиц (2026-08-26). Открытые 🔴/🟠 — блок
> manager-app (MGR-004…MGR-008). Распределение открытых пунктов по потокам —
> в [PARALLEL_WORK.md](PARALLEL_WORK.md).
