...
| MGR-019 | Политики: `can_add_cards=false` (жёстко), пауза воркера (не выдаём срезы, закреплённое остаётся; менеджер может забрать в пул/выгрузить — воркер только бронь и деклайн), вкл/выкл шопа per-воркер, кулдаун после деклайна (окно N минут), лимиты профилей/дропов, пресеты (новичок/проверенный/испытательный), онбординг-мастер, жизненный цикл worker_keys (бан → отзыв среза, restore → перевыпуск)                                                                                                                  | 🟠        | `cc-sync-server`, `manager-app`, `src-tauri`                                           | ⬜       |
