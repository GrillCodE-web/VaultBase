...
| MGR-021 | Инфра-гигиена: retention конвертов на сервере + месячные rollups у менеджера, контракт-тесты телеметрии (golden payload_version, матрица «старый менеджер ↔ новый воркер»), staging-полигон (DEVOPS-002) для миграции пулов                                                                                                                                                                                                                                                                                             | 🟡        | `cc-sync-server`, `e2e`, `docs`                                                        | ⬜       |
...
| ADMIN-001  | Полный редизайн админ-панели (вариант B): дизайн-токены, lucide вместо эмодзи, тосты/скелетоны, релизы карточками по версиям с вкладками воркер/менеджер и rollout-слайдером | 🟡        | `cc-sync-server/admin/*`, `routes/upload.js`                                   | 🔄 @main |
