...
> **Создан:** 2026-08-11 · **Обновлён:** 2026-08-29  
> **Всего пунктов:** 199 · **Выполнено:** 149 (из них 9 «уже было») · **Открыто:** 43  
...
| FEAT-004 | IF-THEN правила автоматизации (status=declined → пометить карту)                   | 🟡        | Новый модуль            | ✅ @main |
...
### 5.5.1 Пул-модель «менеджер — мозг, воркер — рука» (решение владельца 2026-08-29)

Модель: **воркер не создаёт ничего из пула компании — только бронирует карты и кидает в деклайн**. Менеджер (manager-app) — единственный источник карт/прокси/почт: живой пул №1 у менеджера, воркер берёт карту → карта закрепляется за ним эксклюзивно (у других её нет); деклайн → карта уплывает менеджеру в пул №2 (у воркера не остаётся); экспорт пулов (любых) — менеджер выгружает к себе на ПК → зачистка из системы. Забрать в пул/выгрузить может менеджер в обе стороны. Группы карт и pair-коды упраздняются целиком (лишние метаданные на сервере). Все API-ключи: BIN — у менеджера (обогащение карты при импорте, воркер BIN API не дёргает); 17track/stuffer остаются у воркера, менеджер может расшарить любой API-ключ выбранным воркерам. Прокси/email — централизованно, PPTP-секрет воркер вводит сам и наружу не отдаёт. Визуал времени у менеджера — по МСК.

| #       | Задача                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Приоритет | Файл(ы)                                                                                | Статус   |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | -------------------------------------------------------------------------------------- | -------- |
| MGR-014 | Фундамент данных: миграция v21 — `orders.created_by` (+backfill из card_assignments), `order_status_history` (from/to/changed_by/source → time-in-status), `card_status_events` (структурный лог статусов карт вместо парсинга строк activity_log), опциональная причина деклайна (ручной ввод)                                                                                                                                                                                                                         | 🔴        | `_migrations.rs`, `_orders.rs`, `_cards.rs`, `commands/orders.rs`, `commands/cards.rs` | ✅ @main |
| MGR-015 | Telemetry v2: `payload_version` + `worker_sent_at` в конвертах, `tz_offset` в daily_stats (менеджер показывает по МСК), errors_24h по категориям (imap/sync/proxy/order), реальные smtp/proxy health вместо нулей-заглушек, снимок состава пула (BIN/страна/возраст) + by_user-сплит в daily_stats, очередь неотправленных конвертов с капом (офлайн-неделя не теряет стату)                                                                                                                                            | 🔴        | `commands/telemetry.rs`, `_analytics.rs`                                               | ✅ @main |
| MGR-016 | Сервер: worker_keys (X25519-пубключи воркеров по образцу manager_keys), регистрация ключа при активации, доставка запечатанных срезов, запрет worker-создания карт в sync-push (только статусы существующих), гашение легаси-групп/pair-кодов, аудит раздач и экспортов                                                                                                                                                                                                                                                 | 🔴        | `cc-sync-server/*`                                                                     | ✅ @main |
| MGR-017 | manager-app — vault карт: импорт (PAN только у менеджера), дубль-чек против реестра сожжённых/«на руках у воркера»/живого пула, раздача персональных срезов запечатанно под worker_keys, пул №2 (деклайны, счётчик), экспорт любого пула на ПК + зачистка + журнал вывозов, занятость пула/потеряшки/суточная сверка целостности, массовые операции по воркерам, пауза воркера, роли (руководящий/менеджер/смотрящий), недельный дайджест по менеджерам и воркерам                                                      | 🔴        | `manager-app/*`                                                                        | ✅       |
| MGR-018 | Воркер — приёмник: приём запечатанных срезов (`source='manager'`), выпил импорта карт (import_cards, ADD_CARDS_MANUAL, ImportModal), выпил групп/pair-кодов/NOSYNC, центр. срезы прокси/email (PPTP-секрет локально, не синкается), BIN-ключ у менеджера (обогащение при импорте); 17track/stuffer — у воркера + share-ключи от менеджера                                                                                                                                                                               | 🔴        | `src-tauri`, `src/pages/Cards`, `Settings.jsx`, `Onboarding.jsx`, `sync.rs`            | ✅ @main |
| MGR-019 | Политики: `can_add_cards=false` (жёстко), пауза воркера (не выдаём срезы, закреплённое остается; менеджер может забрать в пул/выгрузить — воркер только бронь и деклайн), вкл/выкл шопа per-воркер, кулдаун после деклайна (окно N минут), лимиты профилей/дропов, пресеты (новичок/проверенный/испытательный), онбординг-мастер, жизненный цикл worker_keys (бан → отзыв среза, restore → перевыпуск)                                                                                                                  | 🟠        | `cc-sync-server`, `manager-app`, `src-tauri`                                           | ✅       |
| MGR-020 | Менеджер — умный слой: инсайты RU с ранжированием по объёмному влиянию (без себестоимости — отвергнуто), «Действия дня» (карточка = кнопка), score + адаптивные квоты human-in-the-loop, dual-baseline аномалии (против флота + self 7д), прогноз выгорания пула, дрейф версий, гигиена данных, диплинки из алертов, дельты/спарклайны/тултипы «?»/ночная сводка/пресеты фильтров                                                                                                                                       | 🟠        | `manager-app/*`                                                                        | ✅ @main |
| MGR-021 | Инфра-гигиена: retention конвертов на сервере + месячные rollups у менеджера, контракт-тесты телеметрии (golden payload_version, матрица «старый менеджер ↔ новый воркер»), staging-полигон (DEVOPS-002 — отдельный пункт, ⬜) для миграции пулов                                                                                                                                                                                                                                                                       | 🟡        | `cc-sync-server`, `e2e`, `docs`                                                        | ✅ @main |
| MGR-022 | Статистика A→Я (6 дыр): SLA-таймеры из `order_status_history` (time-in-pending, часы до shipped/delivered; `get_sla_stats` + SLA-блок в daily_stats), сравнение воркеров (`get_fleet_comparison` + таблица в Analytics), воронка taken→used→delivered у менеджера, флотовая теплокарта BIN×шоп (воркер шлёт топ-N пар в daily_stats, менеджер агрегирует), месячные rollups + очистка `reports` у менеджера (часть MGR-021 — серверная retention НЕ входит), полоса дрейфа версий на дашборде менеджера (часть MGR-020) | 🟠        | `src-tauri` (telemetry, _orders, dashboard), `manager-app/*`                           | ✅ @main |

### 5.9 Флоат-окно и воркер-апп (модель «только бронь и деклайн»)

| #         | Задача                                                                                                                                                                                                      | Приоритет | Файл(ы)                                        | Статус |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------------------------------------------- | ------ |
| FLOAT-001 | Копи-варианты полей: отдельные кликабельные чипы «зип / адрес / фуллнейм / имя / фамилия / телефон» (copySensitive с автоочисткой), адрес одной строкой и многострочно по выбору                            | 🟠        | `src/float.jsx`                                | ✅     |
| FLOAT-002 | Быстрая замена карты под тот же зип+бин, или по штату, или ближний штат (если точного нет); быстрая замена курьера по зипу/штату из уже забронированного с автоподстановкой                                 | 🟠        | `float.jsx`, `_orders.rs`, `commands/couriers` | ✅     |
| FLOAT-003 | Таб Email/OTP: последние письма с имейла профиля + автовычлененный код подтверждения (по маске) + «копировать код»                                                                                          | 🟠        | `float.jsx`, `imap.rs`                         | ✅     |
| FLOAT-004 | Быстрый деклайн прямо из флоата: кнопка → причина НЕобязательна (ручной ввод «почему») → карта уезжает менеджеру в пул №2                                                                                   | 🟡        | `float.jsx`, `commands/cards.rs`               | ✅     |
| FLOAT-005 | Поведение: мини-режим (одна строка «номер • CVV • срок»), запоминание позиции окна, глобальный хоткей показать/скрыть                                                                                       | 🟡        | `float.jsx`, `main.rs`                         | ✅     |
| FLOAT-006 | Гигиена: copy billing/shipping через copySensitive (сейчас голый clipboard), лейблы Fresh/Used/Burned и Safe/Warning/High Risk — через i18n (en+ru)                                                         | 🟢        | `float.jsx`                                    | ✅     |
| FEAT-019  | Воркер Dashboard: личная воронка дня (взял → заказы → результат) + операторская аналитика по BIN/шопам/дропам (get_bin_performance/get_shop_win_loss уже require_user; менеджерскую аналитику не дублируем) | 🟡        | `DashboardRedesigned.jsx`, `src/store`         | ✅     |
...
| DEVOPS-002 | Staging окружение: staging sync-сервер + процесс деплоя                                                                                                                      | 🟡        | `VaultBase.staging.toml`                                                       | ✅ @main |
...
| DEVOPS-006 | CI менеджера (`mgr-v*` → 3 ОС → сервер) + фикс release_files: UNIQUE без платформы затирал Windows/macOS-обновления (живо только Linux)                                      | 🔴        | `build-manager-release.yml`, `upload.js`, `database.js`, `upload-artifacts.py` | ✅       |
| ADMIN-001  | Полный редизайн админ-панели (вариант B): дизайн-токены, lucide вместо эмодзи, тосты/скелетоны, релизы карточками по версиям с вкладками воркер/менеджер и rollout-слайдером | 🟡        | `cc-sync-server/admin/*`, `routes/upload.js`                                   | ✅       |
...
## 14. РЕДИЗАЙН 05 «Adaptive»

> Контракт: [docs/REDESIGN_05_PLAN.md](docs/REDESIGN_05_PLAN.md). Выбранный язык —
> вариант 05 (Adaptive), мокап `design-mockups/05-adaptive-full.html`. Поток frontend:
> только `src/**` + `MASTER_CHECKLIST.md`/`SESSION_LOG.md`; `src-tauri/**` не трогать
...
| REDESIGN-05-1   | Каркас: сайдбар 232px с группами, топбар 56px, статус-бар, ⌘K-палитра, единый инспектор, переключатель плотности                                                                                                    | 🔴        | `App.jsx`, `layout.css`, новые компоненты                                                            | ✅ @r              |
| REDESIGN-05-2   | Компоненты: Button/Input/Select/Badge/Table/Panel/Modal/Toast/EmptyState; унификация ~20 модалок в один `<Modal>`; типографика классами                                                                             | 🔴        | `components/`, `components.css`                                                                      | ✅ @r              |
| REDESIGN-05-3   | Страницы по одной: Dashboard → Orders → Cards → Profiles → остальные → auth-контур → float-окно                                                                                                                     | 🔴        | `pages/`, `pages.css`, `float.jsx`                                                                   | ✅ @r              |
| REDESIGN-05-4   | Фичи frontend-only: порции 1–5, статистика/рекомендатель, заметки, FTS-поиск, виртуализация, колонки, фильтры-пресеты                                                                                               | 🟠        | `src/**`                                                                                             | ✅ @r              |
| REDESIGN-05-5   | Фичи, ждущие backend-сессию: пул карт/бронирование, панель воркеров, трекинг-поллер, E2E-чат. Разбито на backend-подпункты 5B1–5B4 (@main); frontend-часть остаётся за @r                                           | 🟠        | стык с backend                                                                                       | ✅ @main (5B1–5B4) |
...
| REDESIGN-05-5B4 | Backend: E2E-чат — opaque-relay + хранилище блобов на сервере, X25519/AES-GCM обвязка воркера, таблицы чатов, команды                                                                                               | 🟠        | `cc-sync-server/ws-tauri.js`, `database.js`, `src-tauri/commands/chat.rs` (новый)                    | ✅ @main (4b92447) |
| REDESIGN-05-6   | Десктоп-натив: трей, автообновление, D&D импорт, пресеты окон, panic-клавиша. Закрыто редакцией 8ba9ce4 (вошла в 2.12.0): автообновление — ручная проверка, «в один клик» не делалось                               | 🟡        | `src-tauri` (стык) + `src/**`                                                                        | ✅ @main (8ba9ce4) |
...
> Пересчитано по фактическим строкам таблиц 2026-09-01 (MGR-018 закрыт,
> e6d23ca). REDESIGN-05-6 закрыт 2026-09-02 (8ba9ce4, релиз 2.12.0).
> Открытые: PERF-010 и CLEAN-003 (🔄 @b).
> Ниже — история закрытий. Открытых 🟠 нет —
> MGR-013 (panic-пароль воркера + удалённый wipe) закрыт 2026-08-28.
> FEAT-006/007 и MGR-009 закрыты 2026-08-28.
> REDESIGN-05-5B1/5B2 закрыты 2026-08-31 (сервер + воркер, e129790).
> REDESIGN-05-5B3 закрыт 2026-08-31 (трекинг-поллер/checkpoints/перебивка, 0d19084).
