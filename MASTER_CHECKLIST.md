...
| #        | Задача                                 | Приоритет | Файл(ы)             | Статус |
| -------- | -------------------------------------- | --------- | ------------------- | ------ |
| FEAT-006 | Напоминание: скоро истечёт срок карты  | 🟡        | `background.rs`, UI | ⬜     |
| FEAT-007 | Напоминание: проверить трекинг посылки | 🟡        | `background.rs`, UI | ⬜     |
| FEAT-008 | Cron-подобные задачи в background.rs   | 🟡        | `background.rs`     | ⬜     |
...
| FEAT-012 | Live тест пишущих методов (add_courier, new_package)                                              | 🟡        | `stuffer.rs`               | ⬜     |
...
| MGR-004 | Воркер: отправка heartbeat + daily_stats запечатанными конвертами (X25519→HKDF→AES-GCM, см. MANAGER_APP.md §2.2, §4)           | 🔴        | `src-tauri/src/commands/telemetry.rs` (новый) | ⬜     |
| MGR-005 | Воркер: применение политик живьём (ban→лок, force_logout, мин. версия, permissions_override, квоты)                            | 🔴        | `src-tauri`, `src/hooks`                      | ⬜     |
...
| MGR-010 | Активация/создание менеджерских лицензий прямо из manager-app (сейчас — только веб-админка)                                    | 🟡        | сервер + manager-app                          | ⬜     |
...
> Подсчитано скриптом по фактическим строкам таблиц (2026-08-25). Открытые 🟠 —
> TEST-001/TEST-002 (E2E flow и CRUD). Распределение открытых пунктов по потокам —
