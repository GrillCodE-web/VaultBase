...
> **Создан:** 2026-08-11 · **Обновлён:** 2026-08-28  
> **Всего пунктов:** 184 · **Выполнено:** 145 (из них 9 «уже было») · **Открыто:** 32  
...
> распределены: 20 → STREAM A (backend), 23 → STREAM B (frontend), 1 → @u (кросс-поток
> uPanel, ветка agent/upanel — FEAT-018).
...
| #        | Задача                                                                             | Приоритет | Файл(ы)                 | Статус   |
| -------- | ---------------------------------------------------------------------------------- | --------- | ----------------------- | -------- |
| FEAT-001 | Авто-архив: перемещение `dead` карт в отдельную директорию по нажатию пользователя | 🟡        | `commands/cards.rs`, UI | ✅ @main |
| FEAT-002 | Risk V2: ML-подобный скоринг (магазин, карта, время суток, сумма)               | 🟡        | `_orders.rs`            | ✅ @main |
| FEAT-003 | Smart-подсказки: «Карта скоро сгорит», «3 неуспешных заказа подряд»             | 🟡        | UI notifications        | 🔄 @a    |
...
| FEAT-011 | Общий список курьеров с индивидуальными API ключами                                               | 🟡        | `stuffer.rs`, UI           | ✅ @main |
...
### 5.8 Внешние API (uPanel PPTP)

| #        | Задача                                                                                                                                                                                                                                                  | Приоритет | Файл(ы)                                                                                                   | Статус |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------- | ------ |
| FEAT-018 | Интеграция uPanel PPTP: в разделе Proxies вкладка PPTP (живые серверы, фильтры, «взять», креды, заметки), прокси-вкладка — заглушка; CRUD подключений (URL + Bearer upl_..., токен шифруется); виджет Online/Offline всех подключённых API на Dashboard | 🔴        | `commands/upanel.rs`, `database/_upanel.rs`, `src/pages/Proxies.jsx`, `src/pages/DashboardRedesigned.jsx` | ✅     |
...
| CLEAN-009 | Prop drilling в Cards: compound components pattern или Context для CardRow     | 🟡        | `Cards/cardRowContext.js`                               | ✅     |
