...
> **Всего пунктов:** 199 · **Выполнено:** 145 (из них 9 «уже было») · **Открыто:** 47  
...
| #        | Задача                                                | Приоритет | Файл(ы)             | Статус   |
| -------- | ----------------------------------------------------- | --------- | ------------------- | -------- |
| FEAT-005 | Еженедельный отчёт: заказы, success rate, расход карт | 🟡        | jsPDF, новый модуль | 🔄 @main |
...
| MGR-019 | Политики: `can_add_cards=false` (жёстко), пауза воркера (не выдаём срезы, закреплённое остается; менеджер может забрать в пул/выгрузить — воркер только бронь и деклайн), вкл/выкл шопа per-воркер, кулдаун после деклайна (окно N минут), лимиты профилей/дропов, пресеты (новичок/проверенный/испытательный), онбординг-мастер, жизненный цикл worker_keys (бан → отзыв среза, restore → перевыпуск)                                                                                                                  | 🟠        | `cc-sync-server`, `manager-app`, `src-tauri`                                           | 🔄 @main |
...
| #         | Задача                                                                                                                                                                                                      | Приоритет | Файл(ы)                                        | Статус   |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------------------------------------------- | -------- |
| FLOAT-001 | Копи-варианты полей: отдельные кликабельные чипы «зип / адрес / фуллнейм / имя / фамилия / телефон» (copySensitive с автоочисткой), адрес одной строкой и многострочно по выбору                            | 🟠        | `src/float.jsx`                                | 🔄 @main |
| FLOAT-002 | Быстрая замена карты под тот же зип+бин, или по штату, или ближний штат (если точного нет); быстрая замена курьера по зипу/штату из уже забронированного с автоподстановкой                                 | 🟠        | `float.jsx`, `_orders.rs`, `commands/couriers` | 🔄 @main |
| FLOAT-003 | Таб Email/OTP: последние письма с имейла профиля + автовычлененный код подтверждения (по маске) + «копировать код»                                                                                          | 🟠        | `float.jsx`, `imap.rs`                         | 🔄 @main |
| FLOAT-004 | Быстрый деклайн прямо из флоата: кнопка → причина НЕобязательна (ручной ввод «почему») → карта уезжает менеджеру в пул №2                                                                                   | 🟡        | `float.jsx`, `commands/cards.rs`               | 🔄 @main |
| FLOAT-005 | Поведение: мини-режим (одна строка «номер • CVV • срок»), запоминание позиции окна, глобальный хоткей показать/скрыть                                                                                       | 🟡        | `float.jsx`, `main.rs`                         | 🔄 @main |
| FLOAT-006 | Гигиена: copy billing/shipping через copySensitive (сейчас голый clipboard), лейблы Fresh/Used/Burned и Safe/Warning/High Risk — через i18n (en+ru)                                                         | 🟢        | `float.jsx`                                    | 🔄 @main |
| FEAT-019  | Воркер Dashboard: личная воронка дня (взял → заказы → результат) + операторская аналитика по BIN/шопам/дропам (get_bin_performance/get_shop_win_loss уже require_user; менеджерскую аналитику не дублируем) | 🟡        | `DashboardRedesigned.jsx`, `src/store`         | 🔄 @main |
...
| REDESIGN-05-2 | Компоненты: Button/Input/Select/Badge/Table/Panel/Modal/Toast/EmptyState; унификация ~20 модалок в один `<Modal>`; типографика классами                                                                             | 🔴        | `components/`, `components.css`                                                                      | ⬜        |
...
| REDESIGN-05-5 | Фичи, ждущие backend-сессию: пул карт/бронирование, панель воркеров, трекинг-поллер, E2E-чат                                                                                                                        | 🟠        | стык с backend                                                                                       | ⬜ (ждёт) |
